...
void CGame::Draw(float x, float y, LPDIRECT3DTEXTURE9 texture, int left, int top, int right, int bottom, D3DXVECTOR2 scale, int alpha, Vector2 pivot)
{
	...
	D3DXVECTOR3 p(int(x - camPos.x - pivot.x), int(y - camPos.y - pivot.y), 0);
	RECT r;
	r.left = left;
	r.top = top;
	r.right = right;
...
bool CGame::Load(string gameFile)
{
	...
	{
		string scenceId = node->Attribute("id");
		string path = node->Attribute("path");
		LPSCENE scene = new CPlayScene(scenceId, path);
		CScences::GetInstance()->Add(scenceId, scene);
	}

	OutputDebugStringW(ToLPCWSTR("ScenceStart id: " + scenceStartId + '\n'));
	

	DebugOut(L"[INFO] Loading game file : %s has been loaded successfully\n", gameFile);
	current_scene = scenceStartId;
